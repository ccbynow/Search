...
class getIndex{
   ...
   {error_reporting(0);
	...
	//搜索指定文件
	function searchText2($file, $keyword)
	{set_time_limit (0);
		$line = 0 ; //初始化行数  
		$flag=true;
		//打开文件  
	...
	//搜索目录下所有文件(正则)
	function searchFile2($dir, $keyword)
	{
		$sFile = $this->getFile($dir);//遍历所有的文件
		...
		$sResult = array();
		foreach ($sFile as $file) {
		if ($this->searchText2($file, $keyword)) {
		$sResult[] = $file;
		}
		}
		if (count($sResult) <= 0) {
...
$fileArr = $index->searchFile2($dir, $keyword);
